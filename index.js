...
// Note: This relies on Node.js v18+ which has built-in 'fetch'.
// If you get "fetch is not defined", upgrade Node.js.

const app = express();
app.use(express.json());
app.use(cors());

// Configure multer to store file in memory for parsing
const upload = multer({ storage: multer.memoryStorage() });

// --- CONFIGURATION ---
const OLLAMA_URL = "http://127.0.0.1:11434/api/generate";
const MODEL_NAME = "llama3"; // <--- CHANGE THIS if you use "llama3.2", "mistral", etc.
// ---------------------
...
app.post('/query', async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({ error: "Failed to connect to Ollama.Is it running?" });
    }
});
